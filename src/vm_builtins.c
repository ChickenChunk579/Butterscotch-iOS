...
static int32_t gmlAsyncBufferKick(Runner* runner, AsyncBufferOp* ops, int32_t opCount, const char* groupName) {
    ...
    if (runner->asyncVideoQueue != nullptr) {
        repeat((int32_t) arrlen(runner->asyncVideoQueue), i) {
            free(runner->asyncVideoQueue[i].type);
        }
        arrfree(runner->asyncVideoQueue);
...
static RValue builtin_video_resume(VMContext* ctx, RValue* args, int32_t argCount) {
    if (g_VideoPlayer.status == VIDEO_STATUS_PAUSED) {
...
static RValue builtin_video_seek_to(VMContext* ctx, RValue* args, int32_t argCount) {
...
static RValue builtin_video_get_status(VMContext* ctx, RValue* args, int32_t argCount) {
    return RValue_makeInt32((int32_t)g_VideoPlayer.status);
}

static RValue builtin_video_get_format(VMContext* ctx, RValue* args, int32_t argCount) {
    return RValue_makeInt32(0); // 0 = RGBA color space fallback
}

static RValue builtin_video_get_duration(VMContext* ctx, RValue* args, int32_t argCount) {
    return RValue_makeReal(g_VideoPlayer.durationSec * 1000.0); // Converted to milliseconds
}

static RValue builtin_video_get_position(VMContext* ctx, RValue* args, int32_t argCount) {
    return RValue_makeReal(g_VideoPlayer.currentPosSec * 1000.0); // Converted to milliseconds
}

static RValue builtin_video_get_volume(VMContext* ctx, RValue* args, int32_t argCount) {
...
static RValue builtin_video_pause(VMContext* ctx, RValue* args, int32_t argCount) {
